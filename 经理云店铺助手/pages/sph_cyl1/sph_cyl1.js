    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          var g_saoall = getCurrentPages()[1].get_saoall();
          ...
          getCurrentPages()[1].set_saoall(g_saoall);
          sao_all = {};
  ...
  onReady: function () {
    ...
    wx.getSystemInfo({
      success: function (res) {
        ...
        var qjLength = Math.floor((res.windowHeight - 185) / Math.ceil(res.windowWidth / 750 * 60));
        that.setData({
          qjLength: qjLength,
          zjArr: util.zjArr.slice(zjIndex, zjIndex + zjLength),
